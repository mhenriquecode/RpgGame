...
public class SpecialEffectPaladin implements SpecialEffect {
    ...
    public int applyEffect(RpgCharacter character, int originalDamage) {
        if(character.getHealth() + originalDamage > character.getMaxHealth()) character.setHealth(character.getMaxHealth());
        else character.setHealth(character.getHealth() + originalDamage);
